...
for x = (-P/2):1:(P/2)-1
     for y = (-Q/2):1:(Q/2)-1
        ...
        D_2 = 60;
        D_3 = 160;
        H_1(x+(P/2)+1,y+(Q/2)+1) = 1/(1+(D/D_1)^2);   
        H_2(x+(P/2)+1,y+(Q/2)+1) = 1/(1+(D/D_2)^2);
        H_3(x+(P/2)+1,y+(Q/2)+1) = 1/(1+(D/D_3)^2);
     end
end
...
subplot(2,2,2);
imshow(H_1,[0 1]);
xlabel('Butterworth Lowpass (D_{0}=30,n=1)');
subplot(2,2,1);
...
subplot(2,2,2);
imshow(H_2,[0 1]);
xlabel('Butterworth Lowpass (D_{0}=60,n=1)');
subplot(2,2,1);
...
subplot(2,2,2);
imshow(H_3,[0 1]);
xlabel('Butterworth Lowpass (D_{0}=160,n=1)');
subplot(2,2,1);
